"""
This is the encryptor microservice for CS361.
...
2.  Run this program as a different process, e.g. "python3 encryptor_service.py"
...
while True:
    time.sleep(2)
    encryptorServiceFile = open('service.txt', 'r+')
    fileText = encryptorServiceFile.read()
    if not isbase64(fileText): # Check file for if contents are already base64 encoded
        ...
        encryptorServiceFile.seek(0)
        encryptorServiceFile.truncate() # Remove data from file
        encryptorServiceFile.write(encryptedFileText.decode()) # Write encoded data to file
    encryptorServiceFile.close()
